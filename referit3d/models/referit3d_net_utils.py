...
def make_batch_keys(args, extras=None):
    ...
    batch_keys = ['tokens', 'target_pos', 'token_inds', 'token_num', 'context_size',
    ...
    if args.img_encoder:
        batch_keys.append('objectsImgs')
    if args.sceneCocoonPath:
        batch_keys.append('sceneImgs')
    if args.twoStreams or not args.img_encoder:
        batch_keys.append('objects')
...
def single_epoch_train(model, data_loader, criteria, optimizer, device, pad_idx, args):
    ...
    if args.train_vis_enc_only == False:
        ...
        referential_loss_mtr_2d = AverageMeter()
        ref_acc_mtr_2d = AverageMeter()
        txt_acc_mtr = AverageMeter()
    if args.context_2d == 'unaligned' and args.obj_cls_alpha > 0:
        cls_acc_mtr_2d = AverageMeter()
        obj_loss_mtr_2d = AverageMeter()
    ...
    if args.train_vis_enc_only:
        ...
        repeatationFactor = 1
    for i in range(repeatationFactor):
        print("repeatationFactor index = ", i)
        ...
        for batch in tqdm.tqdm(data_loader):
            ...
            total_loss_mtr.update(total_loss.detach().item(), batch_size)
    ...
    if args.context_2d == 'unaligned' and args.obj_cls_alpha > 0:
        metrics['train_obj_clf_loss_2d'] = obj_loss_mtr_2d.avg
        metrics['train_object_cls_acc_2d'] = cls_acc_mtr_2d.avg

    if args.train_vis_enc_only == False:
        metrics['train_referential_loss_2d'] = referential_loss_mtr_2d.avg
        metrics['train_referential_acc_2d'] = ref_acc_mtr_2d.avg
        ...
        metrics['train_referential_loss_2d'] = 0
        metrics['train_referential_acc_2d'] = 0
        metrics['train_referential_acc'] = 0
        metrics['train_object_cls_acc'] = cls_acc_mtr_2d.avg
        metrics['train_obj_clf_loss'] = obj_loss_mtr_2d.avg
        metrics['train_referential_loss'] = 0
        metrics['train_txt_cls_acc'] = 0
    return metrics
...
def compute_losses(batch, res, criterion_dict, args):
    ...
    if args.train_vis_enc_only:
        ...
        return {'total_loss': obj_clf_loss_2d, 'referential_loss_2d': 0,
                'obj_clf_loss_2d': obj_clf_loss_2d, 'lang_clf_loss': 0}
    ...
    if args.s_vs_n_weight is not None:
        ...
        total_loss = criterion(logits, batch['target_pos']) * 5
    referential_loss = total_loss.detach().item()

    # Early 3D ref loss:
    if args.twoTrans:
        criterion = criterion_dict['logits_early']
        total_loss += criterion(res['logits3D_beforeFusion'], batch['target_pos'])

    sim_loss_type = 'contrastive'
    if sim_loss_type == 'contrastive':
        simloss = getattr(current_module, 'contrastive_loss')

    referential_loss_2d = 0
    if args.context_2d == 'unaligned':
        ...
        if args.s_vs_n_weight is not None:
            ...
            criterion = criterion_dict['logits_2d']
            vg2d_loss = criterion(res['logits_2D'], batch['target_pos'])
        referential_loss_2d = vg2d_loss.detach().item()
        # Early 2D ref loss:
        if args.twoTrans:
            criterion = criterion_dict['logits_2d_early']
            vg2d_loss += criterion(res['logits2D_beforeFusion'], batch['target_pos'])
    ...
    if args.obj_cls_alpha > 0:
        ...
        if args.context_2d == 'unaligned':
            ...
            total_loss += obj_clf_loss_2d * args.obj_cls_alpha
    ...
    return {'total_loss': total_loss, 'referential_loss': referential_loss, 'referential_loss_2d': referential_loss_2d,
            'obj_clf_loss': obj_clf_loss, 'lang_clf_loss': lang_clf_loss, 'obj_clf_loss_2d': obj_clf_loss_2d}
...
def save_predictions_for_visualization(model, data_loader, device, channel_last, seed=2020):
    ...
    for batch in data_loader:
        ...
        for i in range(batch_size):
            ...
            if res['class_logits_2d'] is not None:
                res_list.append({
                    'scan_id': batch['scan_id'][i],
                    'utterance': batch['utterance'][i],
                    'target_pos': batch['target_pos'][i].cpu(),
                    ...
                    'bboxes': batch['objects_bboxes'][i].cpu().numpy(),
                    'predicted_classes': res['class_logits'][i].argmax(dim=-1).cpu(),
                    'predicted_classes_2d': res['class_logits_2d'][i].argmax(dim=-1).cpu(),
                    'predicted_target_pos': res['logits'][i].argmax(-1).cpu(),
                    'object_ids': batch['object_ids'][i],
                    'context_size': batch['context_size'][i],
                    'is_easy': batch['is_easy'][i]
...
def prediction_stats(logits, gt_labels):
    ...
    mean_accuracy = torch.mean(correct_guessed.double()).detach().item()
